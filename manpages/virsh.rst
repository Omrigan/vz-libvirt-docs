...
relative. Using the *--compress* flag wil turn compression on.
...
.. code-block::
    ...
    x-exec domain [--shell] [--envs env1,env2,...] [--] [cmd]
    [arg1] [arg2] ...

Execute the command *cmd* with the arguments *arg1*, *arg2*, etc. in the
guest. The guest must have the guest agent installed. If *--shell* is set, the
command is executed in a shell environment. A comma-separated list of
environment variables can be passed to the executed process via the *--envs*
option.

If *cmd* is not specified, an interactive shell is run. Other options are not
...
.. code-block::
    ...
    blockjob-pause-x domain path

Pause blockjob of *domain* for disk given by *path*.
...
.. code-block::
    ...
    blockjob-resume-x domain path

Resume blockjob of *domain* for disk given by *path*.
...
::
   ...
   domfsfreeze domain [[--mountpoint] mountpoint...] [--hyperv-vss]
...
If *--hyperv-vss* is specified then freeze is done thru HyperV for Windows guests.
...
this way. Also all domain disks should be on hv-scsi controllers.
...
::
   ...
   domfsthaw domain [[--mountpoint] mountpoint...] [--hyperv-vss]
...
If *--hyperv-vss* is specified then thaw is done thru HyperV for Windows guests.
In this case you don't need agent in guest. Note that this option is not compatible
...
In Virtuozzo Hybrid Server, *--min-guarantee* is supported by QEMU/KVM.
The minimum memory guarantee can also be set automatically with the
*--min-guarantee-auto-vz* flag or as some ratio of current memory using
*--min-guranteee-relative-vz*. These three parameters are mutually exclusive.
...
If *--release-ram* is set then memory on source during postcopy phase
will be released as soon as it transfered to destination. With flag
*--postcopy-start* migration run in postcopy mode right from the
beginning. *--shared-workaround* apply very special steps to migrate
...
The workaround is not possible when postcopy is used.
...
::
   ...
   start domain-name-or-uuid [--console] [--paused]
      ...
      [--pass-fds N,M,...] [--reset-nvram] [--x-exportable]
...
*--x-exportable* is currently required to export disks in the write
mode using *x-block-export-start*. See *x-block-export-start* for details.
...
.. code-block::
    ...
    x-block-set-latency-histogram domain disk [boundaries]
    [--op operation]

Configure latency histogram parameters for the I/O operation *operation* on
the disk *disk* and resets all histogram's bins to zero. *operation* is one of
*read*, *write*, *flush* or *unmap*. If *operation* is omitted, histograms
for all operations are configured. The histogram itself is available
via *domstats*.

*boundaries* is a comma-separated list of histogram interval boundaries, in
...
the intervals [0, 10), [10, 50), [50, 100), and [100, +inf).

If both *boundaries* and *operation* are omitted, then histograms for all
...
.. code-block::
    ...
    x-move-nvram domain path [--format format] [--reuse-external]
    [[--config] [--live] | [--current]]

Move an NVRAM image to the new *path*, optionally changing the image format
to *format*. The NVRAM image is only relocated if the domain is running and an
active configuration is to be updated (the old NVRAM image remains). Otherwise,
the NVRAM path is only updated in domain's persistent configuration.

If *--reuse-external* is specified, a pre-created NVRAM image at *path* is
used.

If *--live* is specified, the running domain is affected.
If *--config* is specified, the domain is affected on its next startup.
If *--current* is specified, the current domain state is affected. This is the
default setting. The *--live* and *--config* flags may be specified at the
same time, while *--current* is exclusive.


x-block-snapshot-create
-----------------------
...
.. code-block::
    ...
    x-block-snapshot-create domain FILE [--quiesce] [--autodelete]
    [--checkpoint] [--bypass-cache]

Creates a block snapshot for *domain* specified in the XML file given by path
*FILE*. The domain must be running. Compared to ordinary disk snapshots,
deleting a block snapshot does not require a lengthy commit operation and is
done immediately.

If *--quiesce* is specified, domain filesystems will be frozen before a
snapshot is created. Freezing requires the guest agent to be up and running,
otherwise creating the snapshot will fail.

If *--autodelete* is specified, then the snapshot will be deleted if the
connection which was used to create it is closed.

If *--checkpoint* is specified, then a checkpoint will be created for this
...
The checkpoint will not be deleted when the block snapshot is deleted.

If *--bypass-cache* is specified, the snapshot will avoid the file system
cache, although this may slow down the operation.

Currently block snapshots are not persistent while checkpoins are. Currently a
...
The list of checkpoints is available in the active XML description of an active
...
checkpoint is at the bottom of the list. Checkpoint metadata is not available.

If an ordinary disk snapshot is created, then checkpoints are only kept on the
...
snapshot keeps the checkpoints.


x-block-snapshot-delete
-----------------------
...
.. code-block::
    ...
    x-block-snapshot-delete domain snapshot-name

Deletes the block snapshot *snapshot-name* of the domain *domain*.


x-block-snapshot-list
---------------------
...
.. code-block::
    ...
    x-block-snapshot-list domain

Lists block snapshots of *domain*.


x-block-snapshot-dumxpml
------------------------
...
.. code-block::
    ...
    x-block-snapshot-dumxpml domain snapshot-name

Dumps the XML description for the block snapshot *snapshot-name* of the domain
*domain*.


x-block-checkpoint-remove
-------------------------
...
.. code-block::
    ...
    x-block-checkpoint-remove domain checkpoint-name

Deletes the checkpoint *checkpoint-name* of the domain *domain*.


x-block-export-start
--------------------
...
.. code-block::
    ...
    x-block-export-start domain FILE

Exports the disks/block snapshots/checkpoints of the domain *domain* as
described in *FILE*. Only exporting through the NBD protocol is currently
supported. The domain must be running. Currently block exports are not
persistent.

Block snaphots can be only exported in the read mode. Disks can be exported in
the write mode only if both *--paused* and *--x-exportable* are set on domain
start and only before the domain is resumed.

The command can be called multiple times but currently with the same export
address and port. Basically it can be used to add more disks/block
snapshots/checkpoints to an export after the first call.
...
.. code-block::
    ...
    x-block-export-stop domain FILE

Deletes disks/block snapshots/checkpoints from the export of the domain
*domain* as described in *FILE*. The domain must be running.

The command can be called multiple times do delete exported objects
incrementally.  All of them can also be deleted in a single call.


vz-domain-guest-net-tune
------------------------
...
.. code-block::
    ...
    vz-domain-guest-net-tune domain [--hostname hostname]
        [--dns-server servers] [--search-domain domains]
        [[--config] [--live] | [--current]]

Sets guest network parameters for the *domain*. *hostname* is a new hostname.
*servers* is a comma-separated list of DNS servers. *domains* is
a comma-separated list of search domains for the DNS name.

If *--live* is specified, affects a running domain.
If *--config* is specified, affects the next startup of a persistent guest.
If *--current* is specified, it is equivalent to either *--live* or
*--config*, depending on the current state of the guest.
The *--live* and *--config* flags may be used at the same time,
but *--current* is exclusive. When no flag is specified, the behavior depends
on the hypervisor driver and is equivalent to *--current* in case of
the QEMU driver.
...
::
   ...
   qemu-monitor-command domain { [--hmp] | [--pretty] [--return-value] [--force] }
...
If *--force* is passed, the command will more likely be executed
even if the domain is occupied by another job.
